...
import { IMAGE_URL, PARTICIPANTS_URL } from '../constants/url'
...
function Room({room}) {
  ...
  const fetchParticipantsCount = async () => {
    try {
      const res = await client.get(PARTICIPANTS_URL+`/${room.room_id}/count`);
      if(res.status === 200){
        setCount(res.data)
      }
    } catch (error) {
  ...
  return (
    <div className="roomListRoom">
      <div className="roomListRoom__header">
        <Link to={`/profile/${room.host}`} href="profile.html" className="roomListRoom__author">
      ...
      <div className="roomListRoom__content">
        <Link to={`/rooms/${room.room_id}`} >{room.title}</Link>
        <p>
          {room.description}
        </p>
      </div>
      <div className="roomListRoom__meta">
        <Link to={`/rooms/${room.room_id}`} className="roomListRoom__joined">
        ...
        <p className="roomListRoom__topic">{room.topic_name}</p>
      </div>
    </div>
  )
}
